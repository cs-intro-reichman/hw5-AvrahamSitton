...
public class Scrabble {
	...
	// static String lastHandString = new String();
	// static String mainHandString = new String();
	...
	public static void init() {
		...
		if (NUM_OF_WORDS < DICTIONARY.length) {
			int j = 1;

			while (DICTIONARY[DICTIONARY.length - j] == null) {
				DICTIONARY[DICTIONARY.length - j] = DICTIONARY[j];
				j++;
	...
	public static int wordScore(String word) {
		...
		for (int i = 0; i < length; i++) {
			...
			score += SCRABBLE_LETTER_VALUES[MyString.indexAt(alphabet, letter)];
		}
		score *= length;
	...
	public static String createHand() {
		...
		newHand = MyString.insertRandomly('a', MyString.insertRandomly('e', newHand));
	...
	public static void playHand(String hand) {
		...
		while (onAir) {
			...
			if (isWordInDictionary(input) && MyString.subsetOf(input, hand)) {
				totalScore += wordScore(input);
				System.out.println(
						input + " earned " + wordScore(input) + " points. Score: " + totalScore + " points");
				onAir = true;
		...
		// String mainHandString = createHand(mainHandString, input);
		if (hand.length() == 0) {
	...
	public static void main(String[] args) {
		...
		// testPlayHands();
		// playGame();
		// playHand(mainHandString);
		init();
		println(DICTIONARY);
		System.out.println(isWordInDictionary("aardvark"));
	}
